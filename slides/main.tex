...
\begin{frame}{What is Reinforcement Learning?}
    ...
    \vspace{0.5cm}
    \begin{itemize}
        \item Reinforcement learning is a learning paradigm based on trial-and-error interaction with an environment.
        \item An agent observes states, selects actions, and receives scalar reward signals.
        \item The objective is to learn a policy that maximizes expected cumulative reward over time.
        \item Training data is generated online by the agent itself rather than from a fixed dataset.
        \item Learning is commonly implemented with deep neural networks optimized using backpropagation.
    \end{itemize}
...
\begin{frame}{Key Challenges in Reinforcement Learning}
    ...
    \vspace{0.5cm}
    \begin{itemize}
        \item Data is non independent and non identically distributed because the agent controls the data collection process through its policy.
        \item Exploration is required to discover rewarding actions, but excessive exploration reduces short term performance.
        \item Rewards are often delayed, obscuring the relationship between actions and their long term consequences.
    \end{itemize}
...
Entities involved:
\begin{itemize}
    \item Agent: the decision making system that selects actions.
    \item Environment: the external system that responds to actions.
\end{itemize}
...
\begin{itemize}
    \item Action (a): a control signal chosen by the agent, discrete or continuous.
    \item State (s): the underlying configuration of the environment.
    \item Reward (r): a scalar signal evaluating the action outcome.
\end{itemize}

\vspace{0.3cm}

\small{Note: an observation is a partial or noisy view of the true environment state.}
...
\begin{frame}{Monte Carlo Tree Search}
    ...
    Core idea:
    \begin{itemize}
        \item Decisions are evaluated by simulating possible future action sequences.
        \item A search tree is grown incrementally from the current state.
        \item Computational effort is focused on actions that appear promising.
    \end{itemize}
    ...
    Algorithm structure:
    \begin{itemize}
        \item The tree alternates between states and actions.
        \item Simulations estimate long term outcomes without requiring a learned model.
        \item Action quality is refined through repeated sampling.
    \end{itemize}
